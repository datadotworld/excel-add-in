...
class AddDataModal extends Component {
  ...
  state = {
    isSubmitting: false,
    name: this.props.options.filename
      ? getFilenameWithoutExtension(this.props.options.filename)
      : '',
  ...
  submit = event => {
    ...
    if (
      this.props.options.filename ||
      this.props.doesFileExist(`${this.getFilename(this.state.name)}.csv`)
    ) {
      // Show warning modal
    ...
    return (
      <Grid className="add-data-modal full-screen-modal show">
          ...
          <form onSubmit={this.submit}>
            {excelApiSupported && (
              <FormGroup>
                ...
                <InputGroup>
                  <FormControl value={range} disabled type="text" />
                </InputGroup>
                <HelpBlock>
                  Select the area to bind in the worksheet, it will be reflected
                  here.
                </HelpBlock>
              </FormGroup>
            )}
            {!excelApiSupported && (
              <div>
                <ControlLabel>Dataset range</ControlLabel>
                <HelpBlock>Select the area to bind in the worksheet.</HelpBlock>
              </div>
            )}
        ...
        >
          <div>
            <strong>
              "{displayName}.csv" already exists on data.world. Do you want to
              replace it?
            </strong>
          </div>
          <div>
            Replacing it will overwrite the file on data.world with the contents
            from {excelApiSupported ? range : 'the selected cell range'}
          </div>
        </WarningModal>
      </Grid>
    );
